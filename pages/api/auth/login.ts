...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: err instanceof Error && err.message.includes('Supabase environment')
        ? 'Supabase configuratie ontbreekt. Zet SUPABASE_URL/NEXT_PUBLIC_SUPABASE_URL en SUPABASE_SERVICE_ROLE_KEY lokaal.'
        : 'Inloggen mislukt door een serverfout.',
    })
  }
}
